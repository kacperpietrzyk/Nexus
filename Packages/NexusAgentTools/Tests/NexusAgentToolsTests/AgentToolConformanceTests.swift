...
@Suite("AgentTool conformance - all 101 core tools")
struct AgentToolConformanceTests {
    ...
    @Test("registry built from CoreTaskTools.all has 101 tools")
    func registryCount() {
        let registry = ToolRegistry(tools: CoreTaskTools.all())

        #expect(registry.tools.count == 101)
        #expect(registry.manifest().tools.count == 101)
    }
