...
@Suite("AgentTool conformance - all 97 core tools")
struct AgentToolConformanceTests {
    ...
    @Test("registry built from CoreTaskTools.all has 97 tools")
    func registryCount() {
        let registry = ToolRegistry(tools: CoreTaskTools.all())

        #expect(registry.tools.count == 97)
        #expect(registry.manifest().tools.count == 97)
    }
