...
@Suite("AgentTool conformance - all 65 core tools")
struct AgentToolConformanceTests {
    ...
    @Test("registry built from CoreTaskTools.all has 65 tools")
    func registryCount() {
        let registry = ToolRegistry(tools: CoreTaskTools.all())

        #expect(registry.tools.count == 65)
        #expect(registry.manifest().tools.count == 65)
    }
