...
@Suite("AgentTool conformance - all 73 core tools")
struct AgentToolConformanceTests {
    ...
    @Test("registry built from CoreTaskTools.all has 73 tools")
    func registryCount() {
        let registry = ToolRegistry(tools: CoreTaskTools.all())

        #expect(registry.tools.count == 73)
        #expect(registry.manifest().tools.count == 73)
    }
