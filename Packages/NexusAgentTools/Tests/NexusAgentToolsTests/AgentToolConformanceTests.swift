...
@Suite("AgentTool conformance - all 46 core tools")
struct AgentToolConformanceTests {
    ...
    @Test("registry built from CoreTaskTools.all has 46 tools")
    func registryCount() {
        let registry = ToolRegistry(tools: CoreTaskTools.all())

        #expect(registry.tools.count == 46)
        #expect(registry.manifest().tools.count == 46)
    }
    ...
    private func isValidToolName(_ name: String) -> Bool {
        ...
        let knownNamespaces: Set<Substring> = [
            ...
            "projects", "agents", "labels", "blocks", "people", "activity", "cycles",
        ]
