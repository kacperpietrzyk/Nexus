...
public final class ContextAssembler {
    ...
    private func renderEntity(_ focus: ContextFocus) -> AssembledContext.Section? {
        ...
        switch kind {
        case .task:
            guard let t = try? agentContext.modelContext.context.fetch(
                FetchDescriptor<TaskItem>(predicate: #Predicate { $0.id == id })).first else { return nil }
            return .init(title: "Focus task", body: "\(t.title)")
        case .project:
            guard let p = try? agentContext.modelContext.context.fetch(
                FetchDescriptor<Project>(predicate: #Predicate { $0.id == id })).first else { return nil }
            return .init(title: "Focus project", body: "\(p.name) [\(p.statusRaw)]")
        case .person:
            guard let p = try? agentContext.modelContext.context.fetch(
                FetchDescriptor<Person>(predicate: #Predicate { $0.id == id })).first else { return nil }
    ...
    private func renderSlice(_ slice: RepoSlice, now: Date) -> AssembledContext.Section? {
        ...
        case .recentActivity(let itemID, let kindRaw, let limit):
            ...
            return .init(title: "Recent activity (\(entries.count))",
